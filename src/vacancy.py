...
class Vacancy:
    ...
    def __init__(self, name: str, link: str, salary: Optional[float | str] = None,
                 responsibility: Optional[str | None] = None, id: Optional[int] = None,) -> None:
        if id is None:
    ...
    def _validate_salary(salary: float | str) -> float | str:
        if isinstance(salary, dict):
            ...
            if salary is None or (isinstance(salary, (int, float)) and salary <= 0) or (
                    isinstance(salary, str) and salary.strip() == ""):
